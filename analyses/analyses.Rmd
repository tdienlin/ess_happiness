...
```{r}
# load packages
packages <- c("animation", "car", "cowplot", "foreign", "haven", "knitr", 
...
p_netuse_adolescents <- make_p("netuse", adolescent = TRUE)
```

## Happiness

```{r}
p_happy <- make_p("happy", save = TRUE)
p_happy_adolescents <- make_p("happy", adolescent = TRUE)
```

## Social Meet-Ups

```{r}
p_sclmeet <- make_p("sclmeet", save = TRUE)
p_sclmeet_adolescents <- make_p("sclmeet", adolescent = TRUE)
```

# Intimate Discussion partners
```{r}
p_inmdisc <- make_p("inmdisc", save = TRUE)
p_inmdisc_adolescents <- make_p("inmdisc", adolescent = TRUE)
```

## Health

```{r}
p_health <- make_p("health", save = TRUE)
p_health_adolescents <- make_p("health", adolescent = TRUE)
```

## Health Problems

```{r}
p_hlthhmp <- make_p("hlthhmp", save = TRUE)
p_hlthhmp_adolescents <- make_p("hlthhmp", adolescent = TRUE)
```
